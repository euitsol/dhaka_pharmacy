@extends('user.layouts.master', ['pageSlug' => 'kyc_verification'])
...
@push('css')
    ...
    @if (!empty($kyc->u_submitted_kyc) && $kyc->u_submitted_kyc->status != -1)
        <style>
            .imagePreviewDiv a {
                display: none;
            }
...
@php
    $submitted_kyc = isset($kyc->u_submitted_kyc) ? $kyc->u_submitted_kyc : null;
@endphp
@section('content')
    <section class="kyc_verification">

        <div class="container">
            <div class="row">
                <div class="col-12">
                    <div class="kyc-title d-flex align-items-center justify-content-between">
                        <h3>{{ __('KYC Verification Center') }}</h3>
                ...
                <div class="row">
                    ...
                    <div class="{{ empty($submitted_kyc) || $submitted_kyc->status == -1 ? 'col-8' : 'col-12' }}">
                        <div class="card bg-transparent">
                            ...
                            @endphp
                            @if (!empty($submitted_kyc) && $submitted_kyc->status == 1)
                                <div class="kyc_varified text-center" style="height: auto">
                                    <img src="{{ asset('default_img/kyc_varified.svg') }}" width="25%" class="py-5"
                                        alt="">
                                </div>
                            @else
                                @if (isset($kyc->form_data))
                                    <form method="POST" action="{{ route('u.kyc.store') }}" autocomplete="off"
                                            ...
                                            @foreach (json_decode($kyc->form_data) as $k => $fd)
                                                ...
                                                @elseif($fd->type == 'image')
                                                    <div class="form-group mb-3">
                                                        ...
                                                        <input {{ $disabled ? 'disabled' : '' }} type="file"
                                                            accept="image/*" name="{{ $fd->field_key }}"
                                                            id="{{ $fd->field_key }}"
                                                ...
                                                @elseif($fd->type == 'image_multiple')
                                                    @if (isset($submitted_kyc->submitted_data) &&
                                                        ...
                                                        @php
                                                            $data = collect(
                                                                json_decode($submitted_kyc->submitted_data, true)[$a],
                                                            );
                                                            $result = '';
                                                            if (!empty($data)) {
                        ...
                        success: function(response) {
                            ...
                            if (response.success) {
                                ...
                                let _url = url.replace('_url', response
                                    .url);
                                if (isMultiple) {
                                    var file = `<div class="form-group mb-3">
                                                <label>{{ _('Uploded file - ${count}') }}</label>
                                                <div class="input-group mb-3">
                ...
                $(document).on("click", ".delete_file", function(e) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            this_.closest('.form-group mb-3').remove();
