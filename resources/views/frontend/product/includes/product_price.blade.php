<div class="product_price mt-3">
    ...
    @else
        @if ($single_product->discount_percentage > 0)
            ...
            <span class="badge bg-danger discount_amount"
                data-discount_precenteage={{ $single_product->discount_percentage }}>
            ...
            <div class="d-block d-md-flex align-items-center justify-content-between">
                <div class="form-group my-4 boxed">
                    @foreach ($single_product->units as $unit)
                        <input type="radio" value="{{ $unit->id }}" name="unit_id" data-id="{{ $unit->id }}"
                            data-name="{{ $unit->name }}" data-total_price="{{ $unit->pivot->price }}"
                            data-total_regular_price="{{ $unit->pivot->price }}" class="item_quantity"
                            id="unit-{{ $loop->index }}" @checked($loop->first)>
                        <label for="unit-{{ $loop->index }}">
                            <img src="{{ $unit->image }}" title="{{ $unit->name }}" alt="{{ $unit->name }}">
                        </label>
                    @endforeach
                </div>
                    ...
                    <div class="form-group">
                        <div class="input-group align-items-center" role="group">
                            ...
                            <input type="text" style="line-height: 1.5"
                                class="form-control text-center quantity_input " name="quantity" value="1"
    ...
    @else
        <div class="add_to_card">
            <button class="cart-btn d-flex align-items-center justify-content-center" type="button"
                data-product_slug="{{ $single_product->slug }}" data-quantity="1"
