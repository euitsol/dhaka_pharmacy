...
<head>
    ...
    <title> @yield('title', 'Hub Dashboard') - Dhaka Pharmacy </title>
    ...
    @stack('css')
    <script>
        const staff_id = `{{ staff() ? staff()->id : false }}`;
...
<body class="white-content dark {{ $class ?? '' }}">
    <div class="wrapper">
        @auth()
            @include('hub.partials.navbars.sidebar')
            <div class="main-panel">
                @include('hub.partials.navbars.navbar')
                <div class="content">
                    @yield('content')
                </div>
                @include('hub.partials.footer')
            </div>
        @endauth
    </div>
