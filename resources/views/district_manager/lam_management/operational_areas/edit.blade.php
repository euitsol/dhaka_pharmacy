    ...
    <div class="row px-3">
        <div class="{{ $document ? 'col-md-8' : 'col-md-12' }}">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST"
                        action="{{ route('dm.lam_area.edit', $lam_area->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group">
                            <label>{{ _('Slug') }}</label>
                            <input type="text" value="{{ $lam_area->slug }}" class="form-control {{ $errors->has('slug') ? ' is-invalid' : '' }}" id="slug" name="slug" placeholder="{{ _('Enter Slug (must be use - on white speace)') }}">
