                ...
                <div class="card-body">
                  <form method="POST" action="{{route('lam_management.local_area_manager.local_area_manager_edit',$lam->id)}}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" value="{{$lam->dm->operation_area->name}}" class="form-control dm_area" disabled>
                    </div>
                    <div class="form-group">
                        <label>{{ __('Local Area Manager Area') }}</label>
                        <select name="osa_id" class="form-control lam_area {{ $errors->has('osa_id') ? ' is-invalid' : '' }}">
                          @foreach ($lam->dm->operation_area->operation_sub_areas as $area)
                            <option {{($lam->osa_id == $area->id) ? 'selected' : ''}} value="{{$area->id}}">{{$area->name}}</option>
                        @endforeach
                        </select>
