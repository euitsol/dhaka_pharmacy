...
@section('title', 'Create Hub Staff')
@section('content')
    <div class="row px-3">
        <div class="{{ $document ? 'col-md-8' : 'col-md-12' }}">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-8">
                            <h4 class="card-title">{{ __('Create Hub Staff') }}</h4>
                        </div>
                ...
                <form method="POST" action="{{ route('hm.hub_staff.hub_staff_create') }}">
                    @csrf
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <select name="hub" id="hub"
                                ...
                                @foreach ($hubs as $hub)
                                    <option {{ old('hub') == $hub->id ? 'selected' : '' }} value="{{ $hub->id }}">
                                        {{ $hub->name }}</option>
                                @endforeach
                            </select>
