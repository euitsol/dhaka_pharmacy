...
@push('css')
    ...
    @if (!empty($kyc->lam_submitted_kyc) && $kyc->lam_submitted_kyc->status != -1)
        <style>
            .imagePreviewDiv a {
                display: none;
            }
...
@php
    $submitted_kyc = isset($kyc->lam_submitted_kyc) ? $kyc->lam_submitted_kyc : null;
@endphp
@section('content')
    <div class="row">
            ...
            <div class="card">
                ...
                @else
                    @if (isset($kyc->form_data))
                        <form method="POST" action="{{ route('lam.kyc.store') }}" autocomplete="off"
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        @else
                                                            <a class="btn btn-info btn-sm"
                                                                href="{{ route('lam.kyc.download.file', base64_encode(json_decode($submitted_kyc->submitted_data)->$a)) }}"><i
                                                ...
                                                @if (isset($submitted_kyc->submitted_data) &&
                                                    ...
                                                    @foreach (json_decode($submitted_kyc->submitted_data)->$a as $url)
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            @else
                                                                <a class="btn btn-info btn-sm"
                                                                    href="{{ route('lam.kyc.download.file', base64_encode($url)) }}"><i
                                                                        class="fa-regular fa-circle-down"></i></a>
                                                            @endif
                                                        </div>
                                                    @endforeach
