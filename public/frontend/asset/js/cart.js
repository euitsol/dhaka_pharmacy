...
function updateCartDisplay(data) {
    var regularPrice,discountedPrice,isChecked,append,selectedUnitTotalDiscountedPrice,selectedUnitId,product;
                            ...
                            <div class="item_price col-sm-4 mt-2 mt-sm-5 col-12 ps-0">
                                ${product.discounted_price != product.price ?
                                    `<h4 class="text-start text-sm-end">
                                        <del class="text-danger">৳ <span class="item_count_regular_price">${Number(cart.selected_unit.price * cart.quantity).toFixed(2)}</span></del>
                                    </h4>`
                                    : ''}
                                <h4 class="text-start text-sm-end">
                                    <span>৳</span> <span class="item_count_price" data-total="${selectedUnitTotalDiscountedPrice}">${Number(selectedUnitTotalDiscountedPrice).toFixed(2)}</span>
                                </h4>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="form-group my-1 boxed">
                            ${product.units.map((unit, u_key) => {
                                ...
                                if(isChecked){
                                    ...
                                    discountedPrice = cart.selected_unit.discounted_price;
                                }else{
                                    regularPrice = product.price * unit.quantity * cart.quantity;
                                    discountedPrice = product.discounted_price * unit.quantity * cart.quantity;
                                }
    ...
    $('.subtotal_price').html(numberFormat(data.data.total_discounted_price, 2));
}
...
function getCartCount() {
    return sessionStorage.getItem('cart_product_count') ? parseInt(sessionStorage.getItem('cart_product_count')) : 0;
}
...
function changeQuantity(element, increment) {
    ...
    CartModule.operations.updateItem({
...
$(document).ready(() => {
    ...
    $(document).on('click', '.cart_remove_btn', function() {
        CartModule.operations.removeItem([$(this).data('atc_id')])
            .done(CartModule.handleResponse)
            .fail(CartModule.handleError);
    });
    ...
    $(document).on('click', '.cart_clear_btn', function() {
        const cartIds = $('.cart_remove_btn').map((i, el) => $(el).data('atc_id')).get();
        CartModule.operations.removeItem(cartIds)
            .done(CartModule.handleResponse)
            .fail(CartModule.handleError);
    });
