...
class UserKycController extends Controller
{
    ...
    {
        $s['submitted_kyc'] = SubmittedKyc::with('creater')->where('type', 'user')->orderBy('status')->latest()->get()->groupBy('status');
        return view('admin.user_management.submitted_kyc.index', $s);
    }
    public function details($id): View
    {
        $data['submitted_kyc'] = SubmittedKyc::with('kyc')->findOrFail($id);
        return view('admin.user_management.submitted_kyc.details', $data);
    }
