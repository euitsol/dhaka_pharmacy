...
class DashboardController extends Controller
{
    ...
    public function dashboard(): View
    {
        ...
        $data['total_current_orders'] = (clone $query)->whereBetween('status', [1, 5])->count();
        $data['last_current_order'] = (clone $query)->whereBetween('status', [1, 5])->first();
        $data['total_previous_orders'] = (clone $query)->where('status', 6)->count();
        $data['total_cancel_orders'] = (clone $query)->where('status', -1)->count();
