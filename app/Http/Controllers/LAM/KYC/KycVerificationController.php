...
class KycVerificationController extends Controller
{
    ...
    public function kyc_store(Request $request)
    {
        ...
        $submitted_kyc = $kyc->lam_submitted_kyc;
                ...
                if ($fd->type == 'text') {
                    ...
                    try {
                        if ($request->hasFile($input_name)) {
                            ...
                            $path = $file->storeAs('local-area-manager/kyc/' . lam()->id . '/' . 'image-single/' . $input_name, $customFileName, 'public');
                        ...
                        if (is_array($request->$input_name)) {
                            foreach ($request->$input_name as $key => $file) {
                                if ($file->isFile()) {
                                    ...
                                    $image_path = $file->storeAs('local-area-manager/kyc/' . lam()->id . '/' . 'image-multiple/' . $input_name, $customFileName, 'public');
                    ...
                    if (isset($request->$input_name['url']) && Storage::exists($request->$input_name['url'])) {
                        ...
                        $directoryPath = 'public/local-area-manager/kyc/' . lam()->id . '/' . 'single-uploads';
                        ...
                        $databasePath = 'local-area-manager/kyc/' . lam()->id . '/' . 'single-uploads/' . $new_filename;
                    ...
                    if (isset($request->$input_name) && !empty($request->$input_name)) {
                        ...
                        foreach ($request->$input_name as $key => $input_data) {
                            ...
                            $directoryPath = 'public/local-area-manager/kyc/' . lam()->id . '/' . 'multiple-uploads';
                            ...
                            $sourceFilePath = $file_path;
                            $destinationFilePath = $directoryPath . '/' . $new_filename;
                            $databasePath = 'local-area-manager/kyc/' . lam()->id . '/' . 'multiple-uploads/' . $new_filename;
    ...
    public function file_upload(Request $request)
    {
        if ($request->hasFile('file')) {
            ...
            $filePath = $file->store('public/local-area-manager/kyc/' . lam()->id . '/' . 'uploads');
