...
namespace App\Http\Controllers\Rider\KYC;
...
class KycVerificationController extends Controller
{
    ...
    public function kyc_verification(){
        $data['details'] = KycSetting::where('type', 'rider')->activated()->first();
        $data['datas'] = SubmittedKyc::where('type', 'rider')->where('creater_id', rider()->id)->where('creater_type', get_class(rider()))->first();
        return view('rider.kyc_verification_center.index', $data);
    }

    public function kyc_store(Request $request){
        $kyc_setting = KycSetting::where('type', 'rider')->activated()->first();
        $submitted_kyc = SubmittedKyc::where('type', 'rider')->where('creater_id', rider()->id)->where('creater_type', get_class(rider()))->first();
				...
				if ($fd->type == 'text') {
                    ...
                    try{
                        if($request->hasFile($input_name)){
                            ...
                            $path = $file->storeAs('rider/kyc/'.rider()->id.'/'.'image-single/'.$input_name, $customFileName,'public');
                        ...
                        if (is_array($request->$input_name)) {
                            foreach($request->$input_name as $key=>$file){
                                if ($file->isFile()) {
                                    ...
                                    $image_path = $file->storeAs('rider/kyc/'.rider()->id.'/'.'image-multiple/'.$input_name, $customFileName,'public');
                    ...
                    if (isset($request->$input_name['url']) && Storage::exists($request->$input_name['url'])) {
                        ...
                        $directoryPath = 'public/rider/kyc/'.rider()->id.'/'.'single-uploads';
                        ...
                        $databasePath = 'rider/kyc/'.rider()->id.'/'.'single-uploads/' . $new_filename;
                    ...
                    if (isset($request->$input_name) && !empty($request->$input_name)) {
                        ...
                        foreach ($request->$input_name as $key => $input_data) {
                            ...
                            $directoryPath = 'public/rider/kyc/'.rider()->id.'/'.'multiple-uploads';
                            ...
                            $sourceFilePath = $file_path;
                            $destinationFilePath = $directoryPath . '/' . $new_filename;
                            $databasePath = 'rider/kyc/'.rider()->id.'/'.'multiple-uploads/' . $new_filename;
        ...
        if($submitted_kyc){
            $submitted_kyc->type = 'rider';
            $submitted_kyc->creater()->associate(rider());
    ...
    public function file_upload(Request $request){
        if ($request->hasFile('file')) {
            ...
            $filePath = $file->store('public/rider/kyc/'.rider()->id.'/'.'uploads');
